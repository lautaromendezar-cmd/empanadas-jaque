...
/**
 * Los fallos se OLVIDAN pasada esta ventana sin intentos. Sin esto, el contador
 * sólo sube: una vez alcanzado el máximo, un único error de tipeo meses después
 * volvía a disparar el bloqueo entero. Le pasó al dueño en la primera semana.
 */
const VENTANA_MS = 10 * 60 * 1000;

/** Castigo progresivo a partir del 5º fallo: 30 s, 1, 2, 4 min, tope 5 min. */
const BLOQUEO_BASE_MS = 30 * 1000;
const BLOQUEO_MAX_MS = 5 * 60 * 1000;
...
/** Devuelve el registro de la IP, o null si ya venció la ventana. */
function registroVigente(ip) {
  const reg = intentos.get(ip);
  if (!reg) return null;
  if (Date.now() - reg.ultimo > VENTANA_MS) {
    intentos.delete(ip);
    return null;
  }
  return reg;
...
function castigoDe(fallos) {
  const factor = Math.pow(2, fallos - MAX_INTENTOS);
  return Math.min(BLOQUEO_BASE_MS * factor, BLOQUEO_MAX_MS);
}

/** Segundos que faltan para poder reintentar. 0 = no está bloqueado. */
function bloqueado(req) {
  const reg = registroVigente(ipDe(req));
  if (!reg || reg.fallos < MAX_INTENTOS) return 0;
  const restante = reg.ultimo + castigoDe(reg.fallos) - Date.now();
  return restante > 0 ? Math.ceil(restante / 1000) : 0;
}

function registrarFallo(req) {
  const ip = ipDe(req);
  const reg = registroVigente(ip) || { fallos: 0, ultimo: 0 };
  reg.fallos += 1;
  reg.ultimo = Date.now();
  intentos.set(ip, reg);
